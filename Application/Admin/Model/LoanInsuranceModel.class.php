 ...
 * @adtime 2017-07-14 18:59:59
 */
class LoanInsuranceModel extends \Common\Model\AllModel{
    ...
    public function create_info($name = ''){
        ...
        if($id){
            $log = new LogModel();
            $log->create_log('loan_insurance', $id);
        }
        return $id;
    }
    ...
    public function save_info($id = 0,$name = ''){
        ...
        if($success){
            $log = new LogModel();
            $log->update_log('loan_insurance', $info['id'], array('name'=>$name), array('name'=>$info['name']));
        }
        return $success;
    }
    ...
    public function delete_info($id = 0) {
        ...
        if ($success) {
            $log = new LogModel();
            $log->delete_log('loan_insurance', $id, $info['status']);
        }
        return $success;
    }
}