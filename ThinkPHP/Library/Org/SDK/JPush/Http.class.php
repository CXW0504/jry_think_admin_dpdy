...
use Org\SDK\JPush\Exceptions\APIConnectionException;
use Org\SDK\JPush\Exceptions\APIRequestException;
...
final class Http {
    ...
    public static function log($client, $content) {
        if (!is_null($client->getLogFile())) {
            if(!is_dir(dirname($client->getLogFile()))){
                mkdir(dirname($client->getLogFile()),0777,true);
            }
            error_log($content . "\r\n", 3, $client->getLogFile());
        }
    }
