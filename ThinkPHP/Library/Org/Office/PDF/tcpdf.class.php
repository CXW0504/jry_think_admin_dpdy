...
class tcpdf {
	...
	/**
	 * 从当前位置打印文本<br />
	 * @param $h (float) 距离上一行的高度[上下行距]
	 * @param $txt (string) 添加的字符串
	 * @param $link (mixed) 通过addlink()返回的URL或标识符
	 * @param $fill (boolean) 指示单元格背景是否必须被绘制（true）或透明（false）.
	 * @param $align (string) 文本对齐方式<ul>
         *          <li>L or empty string: 左对齐</li>
         *          <li>C: 居中对齐</li>
         *          <li>R: 右对齐</li>
         *          <li>J: 通栏对齐</li></ul>
	 * @param $ln (boolean) 如果TRUE设置光标在该行的底部，否则在该行的顶部设置光标
	 * @param $stretch (int) 字体缩放模式：<ul>
         *          <li>0 = 禁用缩放</li>
         *          <li>1 = 仅当文本大于单元格宽度时才进行水平缩放</li>
         *          <li>2 = 强制水平缩放以适应单元格宽度</li>
         *          <li>3 = 仅当文本大于单元格宽度时才设置字符间距。</li>
         *          <li>4 = 强制字符间距以适应单元格宽度</li></ul>
         *      一般字体拉伸和缩放值将在可能的情况下保存。
	 * @param $firstline (boolean) 如果TRUE不自动换行，将多出的一部分返回出去。
	 * @param $firstblock (boolean) 如果TRUE，自动换行。
	 * @param $maxh (float) 最大高度。它应该是H = H，而页面底部的剩余空间小于，或者禁用此特性的0。
	 * @param $wadj (float) 第一行宽度将减少到这个数量（用于HTML模式）。
	 * @param $margin (array) 父容器的边界数组
	 * @return mixed 如果$firstline = true返回单元格的数量或剩余的字符串
	 * @public
	 * @since 1.5
	 */
